...
import { coinPost, uncoinPost } from "../../redux/actions/dataActions";

const GiveCoinButton = (props) => {
  const { postId, recipient, coins, classes } = props;

  const giftedCoins = (coins) => {
    if (coins) {
      return coins.filter((coin) => coin.postId === postId);
    } else {
      return 0;
    }
  };
  ...
  const coinPost = () => {
    if (giftedCoins(coins).length < 3) {
      props.coinPost(postId, recipient);
    } else {
      props.uncoinPost(postId, recipient);
    }
  };
  ...
  const coinsIcons =
    giftedCoins(coins).length > 0 ? (
      <div className={classes.giftedCoins}>
        <Typography variant="body1" color="primary">
          +{giftedCoins(coins).length}
        </Typography>
      </div>
    ) : null;
  ...
  return (
    <div className={classes.container}>
      <Button className={classes.footerButton} onClick={coinPost}>
        {giftedCoins(coins).length < 3 ? "Coin" : "unCoin"}
      </Button>
      {coinsIcons}
    </div>
  );
...
const mapActionsToProps = { coinPost, uncoinPost };

export default connect(
  mapStateToProps,
  mapActionsToProps
)(withStyles(styles, { withTheme: true })(GiveCoinButton));
